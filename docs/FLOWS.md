...
**Route groups (2026-08-18):** every page under `src/app/[locale]/` now sits inside one of three route groups — `(marketing)/`, `(auth)/`, or `(app)/` — which only organize files, not URLs (a route group's parentheses never appear in the actual path). File-path references throughout this doc that predate the split (e.g. `src/app/[locale]/login/page.tsx`) are missing the `(auth)/` etc. segment; mentally insert it, or see `git log --diff-filter=R -- 'src/app/[locale]/**'` for the exact moves. Not swept file-by-file across this doc — a good candidate for a follow-up pass.
...
- **Route gating**: `src/middleware.ts`. Combines `next-auth/middleware`'s `withAuth` with `next-intl`'s middleware (`middleware.ts:6-40`). Role checks, in order (must stay in this order — more specific first):
  ...
  - Client-side, `src/app/[locale]/(app)/layout.tsx` does one `useSession` + role/pathname check for all 9 pages under the `(app)` route group (dashboard, dashboard/profile, dashboard/cleaner + edit + disputes, admin + disputes + cancellations + users) — belt-and-suspenders for the flash-of-unauthorized-content case while the session resolves, not the actual security boundary (middleware is). This used to be duplicated near-identically in each page; centralized into the layout so it's one check instead of nine.
...
- **Account deletion**: `DELETE /api/user/me`, entry point on `/dashboard/profile` via `DeleteAccountModal` (type `DELETE` to confirm). Anonymizes in place rather than hard-deleting — `users.deleted_at` stamped, `full_name`/`email`/`phone`/`avatar_url`/`stripe_customer_id`/`password_hash` overwritten so bookings/payments/reviews/disputes keep a valid FK under a "Deleted user" placeholder; `addresses` rows deleted outright (pure PII, nothing else references them — bookings snapshot their own address copy). Cleaners additionally get `cleaner_profiles` anonymized + `status → 'SUSPENDED'`, and their `id_photo_url`/`selfie_photo_url`/profile+cover photos are removed from Storage, not just nulled. Returns 409 (not deleted) if the account has an active `REQUESTED`/`CONFIRMED` booking, an `OPEN` dispute, or a `REFUND_FAILED` payment — resolve those first. `config.ts`'s `authorize()` treats `deleted_at != null` as "no such user." No delayed/scheduled anonymization window (e.g. a 30-day grace period) — that's deferred pending a confirmed legal retention period from the entity/tax advisor, runs immediately for now.
