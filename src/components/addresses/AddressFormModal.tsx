  ...
  return (
    <FullScreenModal isOpen={isOpen} onClose={onClose}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
        ...
        {addresses.length > 0 && (
          <div className="space-y-1.5">
            {addresses.map(a => (
              <div
                key={a.id}
                className="flex items-center justify-between gap-2 rounded-[10px] border border-[#E0EDEC] dark:border-[#253634] px-3 py-2.5"
              >
                <button
                  type="button"
                  onClick={() => mode === 'manage' ? startEdit(a) : onSelect?.(a)}
                  className="flex-1 min-w-0 text-left text-[13px] text-[#0D1F1E] dark:text-[#ECF3F2] hover:text-[#19706A] transition-colors truncate"
                >
                ...
                {mode === 'manage' && !a.is_default && (
                  <button
                    ...
                    className="shrink-0 text-[11px] font-medium text-[#5B7472] dark:text-[#9BB0AE] hover:text-[#19706A] transition-colors disabled:opacity-50"
                  >
                ...
                <button
                  ...
                  className="shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-[#5B7472] dark:text-[#9BB0AE] hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50 text-[16px] leading-none"
                >
                  {deletingId === a.id ? '…' : '×'}
                </button>
              </div>
        ...
        <form onSubmit={handleSave} className="space-y-3 pt-2 border-t border-[#E0EDEC] dark:border-[#253634]">
          <div className="flex items-center justify-between pt-2">
            <p className="text-[13px] font-medium text-[#0D1F1E] dark:text-[#ECF3F2]">{editingId ? tAddr('editAddress') : tAddr('addNew')}</p>
            {editingId && (
              <button type="button" onClick={cancelEdit} className="text-[12px] text-[#5B7472] dark:text-[#9BB0AE] hover:text-[#0D1F1E] dark:hover:text-[#ECF3F2] transition-colors">
                {tAddr('cancelEdit')}
              </button>
            )}
          </div>
          <div>
            <label htmlFor="address-label" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('label')}</label>
            <input
              id="address-label"
              type="text"
              value={label}
          ...
          <div>
            <label htmlFor="address-line1" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('line1')}</label>
            <input
              id="address-line1"
              type="text"
              value={line1}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label htmlFor="address-city" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('city')}</label>
              <select
                id="address-city"
                value={city}
            ...
            <div>
              <label htmlFor="address-postal-code" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('postalCode')}</label>
              <input
          ...
          <div>
            <label htmlFor="address-area" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('area')}</label>
            <input
              id="address-area"
              type="text"
              value={area}
          ...
          <div>
            <label className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('mapPin')}</label>
            <AddressMapPicker
              lat={pin?.lat ?? null}
              lng={pin?.lng ?? null}
          ...
          <div>
            <label htmlFor="address-finding-us-notes" className="block text-[11px] text-[#5B7472] dark:text-[#9BB0AE] mb-1">{tAddr('findingUsNotes')}</label>
            <textarea
