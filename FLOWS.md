...
- **Tab destinations, for a logged-in user**: Home → dashboard root (`/dashboard` or `/dashboard/cleaner`, no query) · Search → `/dashboard/search` (new page, added 2026-08-19 — an app-shell-wrapped copy of the public directory, see below) · Bookings → dashboard root `?tab=bookings` · Messages → dashboard root `?tab=messages` · Profile → `/dashboard/profile`. Home and Bookings land on the same underlying screen (the dashboard root already defaults to showing bookings — there's no separate "overview" page to send Home to) — they're told apart only by which query shape each tab's own link produces (Home always navigates bare, Bookings always sets `?tab=bookings` explicitly), so only one of the two ever highlights as active at a time.
...
| `/dashboard/search` | App-shell copy of the public cleaner directory (added 2026-08-19) — same `CleanersDirectoryView` component, reached via `BottomTabBar`'s Search tab | Authenticated | Filter/sort, favorite, start a team-booking selection | `/cleaners/[slug]`, `/cleaners/book-team` |
