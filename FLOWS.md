...
4. **Multi-cleaner bookings can be created and confirmed, but never pay out or resolve a dispute correctly.** See §11's "Multi-cleaner bookings" section for the full breakdown — `payouts.ts` has no `booking_assignments` awareness (payouts sit `PENDING` forever), the admin disputes UI has no per-assignment resolution view, and no-show marking has no UI at all despite the schema supporting it.
...
**Known gaps, not yet built**: `src/lib/payouts.ts` has no `booking_assignments` awareness yet — a multi-cleaner booking's payouts never release (each assignment sits at `payout_status: PENDING` forever) until that stage ships. The admin disputes UI has no per-assignment resolution view yet, so a dispute on a multi-cleaner booking can be filed (the claim itself works unchanged) but not correctly resolved per-cleaner. No-show marking has no UI at all (`booking_assignments.no_show` exists in schema only). Any assigned cleaner marking COMPLETE completes the whole booking for everyone — there's no partial-completion UX yet even though the schema supports per-assignment tracking.
