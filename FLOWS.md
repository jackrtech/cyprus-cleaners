...
**Not yet verified live** — typechecks clean, but the `no_show_flags`/`no_show_corroborations` migration hasn't been applied to the DB yet (this machine can't reach the Supabase direct-connection host, see below) and the full flag → contest/corroborate → resolve path hasn't been run against a real booking. Needs a live pass (file a flag, contest, corroborate, resolve all three ways) before this is trusted the way the rest of §11 is.
