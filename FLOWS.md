...
### Multi-cleaner bookings — schema groundwork only, no application code yet
Added 2026-08-19 per the deep-planning session Jack's own Todoist task called for before any building starts. `bookings.cleaner_profile_id` and `bookings.introduction_id` are now nullable — `NULL` on both is the discriminator for "this is a multi-cleaner booking" (a single-cleaner booking, still the only kind that actually exists in the app today, always has both set, unchanged). `booking_assignments` holds one row per (booking, assigned cleaner) — own `tier_rate_eur`, own `platform_fee_eur` (charged per cleaner, not once per booking), own payout tracking (`payout_status`/`cleaner_payout_eur`/etc., mirroring `payments`' columns), own `introduction_id` (each assigned cleaner keeps their own normal 1:1 chat thread with the customer — no group chat concept was introduced). `disputes.cleaner_profile_id` is now nullable too: `NULL` means a whole-job dispute against a multi-cleaner booking (per Jack's 2026-08-19 decision, a dispute is always filed against the whole job, never a specific cleaner) — `dispute_assignment_outcomes` is where an admin's per-cleaner resolution/refund-percentage ruling would then live, one row per assigned cleaner, separate from `disputes.resolution`/`refund_percentage` (which stay exactly as-is for ordinary single-cleaner disputes). **None of this is wired up yet** — no booking-creation UI, no `POST /api/bookings/multi` route, no `payouts.ts` awareness of `booking_assignments`, no admin dispute UI for per-assignment outcomes. Building it out is future work, staged (schema → booking-creation flow → payouts → dispute-resolution UI → no-show marking).
