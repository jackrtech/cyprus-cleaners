...
4. **Multi-cleaner bookings work end-to-end — create, confirm, pay out, dispute and resolve — except no-show marking.** See §11's "Multi-cleaner bookings" section for the full breakdown. `booking_assignments.no_show` exists in schema and the payout logic already reads it, but nothing in the app ever sets it to `true`.
...
**Known gaps, not yet built**: no-show marking has no UI at all (`booking_assignments.no_show` exists in schema and is read by the payout logic above, but nothing ever sets it to `true`). Any assigned cleaner marking COMPLETE completes the whole booking for everyone — there's no partial-completion UX yet even though the schema supports per-assignment tracking.
