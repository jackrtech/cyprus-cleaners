...
- **Failure** — a failed transfer sets `payout_status = 'FAILED'` (doesn't unwind anything else) and alerts admin by email. `releaseDuePayouts()` never re-scans `FAILED` rows on its own (only `PENDING`/`BLOCKED`), so nothing retries automatically. **Admin recovery, added 2026-08-19**: `/admin/payouts` (nav tab "Failed payouts") lists every `FAILED` payout across both single-cleaner (`payments`) and multi-cleaner (`booking_assignments`) bookings, each with a manual retry button — `POST /api/admin/payments/[id]/retry-payout` / `POST /api/admin/booking-assignments/[id]/retry-payout`, both thin wrappers around `retryFailedPayout()`/`retryFailedAssignmentPayout()` in `src/lib/payouts.ts`. A retry re-attempts the transfer for the already-computed amount using the *same* idempotency key as the original attempt (`payout-${bookingId}` / `payout-${bookingId}-${cleanerProfileId}`) — if the original attempt actually succeeded at Stripe but the DB update failed, the retry returns the existing transfer instead of creating a second one. Fixes a real gap discovered live 2026-08-19: 4 real completed bookings had been sitting `FAILED` with no way to ever retry them (root cause was the platform's Stripe account settling in GBP while payouts are created in EUR — see the "URGENT INVESTIGATE" Todoist task; this retry mechanism doesn't fix that underlying currency mismatch, it just makes any FAILED payout — for this or any other transient reason — recoverable without a direct DB/Stripe-dashboard intervention).
...
~~4. Multi-cleaner bookings work end-to-end except no-show marking.~~ **Resolved 2026-08-19** — admin-only no-show marking UI shipped at `/admin/team-bookings`. See §11's "Multi-cleaner bookings" section for the full breakdown.
...
**No-show marking** (added 2026-08-19, stage 5): admin-only page at `/admin/team-bookings` (nav tab "Team jobs") lists every COMPLETED multi-cleaner booking with its assignments, each with a no-show checkbox. `PATCH /api/admin/booking-assignments/[id]` flips `booking_assignments.no_show`, guarded to only allow the change while that assignment's own `payout_status` is still `PENDING`/`BLOCKED` — once a payout has actually gone out (or definitively failed) the toggle 409s, since this app has no clawback mechanism. No cleaner/customer self-service or contest flow — admin-only v1, same as the original plan flagged. Verified live end-to-end: created a real multi-cleaner booking, force-completed it, confirmed both assignments render on the page with correct rates, toggled no-show true via the API (reflected immediately), then confirmed the guard correctly rejects the change (409) once payout_status is set to PAID.
