...
4. **Multi-cleaner bookings can be created, confirmed, and paid out, but a dispute on one can't yet be resolved correctly.** See §11's "Multi-cleaner bookings" section for the full breakdown — the admin disputes UI has no per-assignment resolution view (`dispute_assignment_outcomes` has no writer), and no-show marking has no UI at all despite the schema and payout logic both supporting it.
...
**Known gaps, not yet built**: the admin disputes UI has no per-assignment resolution view yet, so a dispute on a multi-cleaner booking can be filed (the claim itself works unchanged) but not correctly resolved per-cleaner (`dispute_assignment_outcomes` has no writer). No-show marking has no UI at all (`booking_assignments.no_show` exists in schema and is read by the payout logic above, but nothing ever sets it to `true`). Any assigned cleaner marking COMPLETE completes the whole booking for everyone — there's no partial-completion UX yet even though the schema supports per-assignment tracking.
